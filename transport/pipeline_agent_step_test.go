...
func TestPipelineAgentStepRunsHeadlessTurn(t *testing.T) {
	...
	out, _, err := h.app.RunHeadlessTurn(ctx, "lint all files in the workspace", "", domain.TrustSafe, nil, "", nil)
	if err != nil {
...
func TestPipelineAgentStepRespectsModelField(t *testing.T) {
	...
	out, _, err := h.app.RunHeadlessTurn(ctx, "run checks", "", domain.TrustSafe, nil, "", nil)
	if err != nil {
		t.Fatalf("RunHeadlessTurn: %v", err)
	}
	text, _ := out["output"].(string)
...
func TestPipelineAgentStepFailsWithoutProvider(t *testing.T) {
	...
	_, _, err := h.app.RunHeadlessTurn(ctx, "do work", "", domain.TrustSafe, nil, "", nil)
...
func TestPipelineAgentStepDoesNotAppearInRoomList(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	_, convID, err := h.app.RunHeadlessTurn(ctx, "lint the workspace", "", domain.TrustSafe, nil, "", nil)
	if err != nil {
		t.Fatalf("RunHeadlessTurn: %v", err)
	}
	if convID == "" {
...
func TestPipelineAgentStepRendersEventPlaceholders(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	out, convID, err := h.app.RunHeadlessTurn(ctx, rendered, "", domain.TrustSafe, nil, "", nil)
	if err != nil {
		t.Fatalf("RunHeadlessTurn: %v", err)
	}
