...
func TestJobRunSkip(t *testing.T) {
	jr := &JobRun{TaskState: TaskState[RunStatus]{Status: StatusQueued}}
	now := time.Date(2026, 9, 1, 12, 0, 0, 0, time.UTC)
	jr.Skip(now)
	if jr.Status != StatusSkipped {
		t.Fatalf("Status = %v, want %v", jr.Status, StatusSkipped)
	}
	if jr.FinishedAt.IsZero() || !jr.FinishedAt.Equal(now) {
...
func TestJobRunBeginRunning(t *testing.T) {
	...
	if jr.StartedAt.IsZero() || !jr.StartedAt.Equal(now) {
		t.Fatalf("StartedAt = %v, want %v", jr.StartedAt, now)
	}
}

func TestJobRunSucceed(t *testing.T) {
	jr := &JobRun{TaskState: TaskState[RunStatus]{Status: StatusRunning}}
	now := time.Date(2026, 9, 1, 12, 0, 0, 0, time.UTC)
...
func TestJobRunFail(t *testing.T) {
	...
	if jr.Error != "build error" {
		t.Fatalf("Error = %q, want %q", jr.Error, "build error")
	}
	if jr.FinishedAt.IsZero() || !jr.FinishedAt.Equal(now) {
		t.Fatalf("FinishedAt = %v, want %v", jr.FinishedAt, now)
	}
...
func TestStepRunBeginRunning(t *testing.T) {
	sr := &StepRun{TaskState: TaskState[RunStatus]{Status: StatusQueued}}
	now := time.Date(2026, 9, 1, 12, 0, 0, 0, time.UTC)
	sr.BeginRunning(now)
	if sr.Status != StatusRunning {
		t.Fatalf("Status = %v, want %v", sr.Status, StatusRunning)
	}
	if sr.StartedAt.IsZero() || !sr.StartedAt.Equal(now) {
		t.Fatalf("StartedAt = %v, want %v", sr.StartedAt, now)
	}
}

func TestStepRunFail(t *testing.T) {
	sr := &StepRun{TaskState: TaskState[RunStatus]{Status: StatusRunning}}
	now := time.Date(2026, 9, 1, 12, 0, 0, 0, time.UTC)
	...
	if sr.FinishedAt.IsZero() || !sr.FinishedAt.Equal(now) {
