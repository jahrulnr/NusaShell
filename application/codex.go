...
// ensureCodexProvider returns the provider record for Codex auth RPCs.
// The Providers UI shows a built-in Codex card before any save, so the first
// login/import must materialize id "codex" from the known builtin defaults.
func (a *App) ensureCodexProvider(providerID string) (*domain.Provider, *contracts.RPCError) {
	if a == nil || a.Providers == nil {
		return nil, &contracts.RPCError{Code: contracts.CodeInternal, Message: "providers store is not configured"}
	}
	...
	if saveErr := a.Providers.Save(p); saveErr != nil {
		return nil, rpcInternal(saveErr)
	}
	a.log("info", "ai", "materialized builtin Codex provider on first auth")
	return p, nil
}
...
func (a *App) markProviderHasCreds(providerID string, hasKey bool) {
	p, err := a.Providers.Get(providerID)
	if err != nil {
		// First auth against the virtual builtin card: create the record so
		// HasAPIKey/configured stick after login/import.
		if providerID == "codex" {
			var rpcErr *contracts.RPCError
			p, rpcErr = a.ensureCodexProvider(providerID)
			if rpcErr != nil {
				a.log("warn", "ai", "failed to materialize provider %s for cred flag update: %s", providerID, rpcErr.Message)
				return
			}
		} else {
...
func (a *App) handleCodexLogin(req contracts.CodexLoginRequest) (any, *contracts.RPCError) {
	if _, rpcErr := a.ensureCodexProvider(req.ProviderID); rpcErr != nil {
		return nil, rpcErr
	}
	if a.CodexOAuth == nil {
...
// deleteCodexAccountCredentials removes multi-account credential keys for a
// deleted provider. Called after the active provider credential is removed.
func (a *App) deleteCodexAccountCredentials(providerID, providerName string) {
	if a.Credentials == nil || providerID == "" {
		return
	}
