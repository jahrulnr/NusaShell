...
var dispatchFamilies = []dispatchFamily{
	...
	},
	{
		...
		def: ToolInfo{
			Name:        "skill",
			Description: "Skill library; \"op\" selects: list {limit?,status?} returns routable (trusted|validated) skills by default; search {query,limit?,status?} returns discovery metadata only (id/name/description/owned_by/status), never SKILL.md content — after selecting a skill, MUST read its absolute SKILL.md with file_read before applying it; save {name,content,description?,id?,path?} creates a learned experimental skill, versions an existing learned experimental skill, or writes a support file when path is set (skill must exist and be agent-mutable); delete {id,owned_by?} removes a learned candidate/experimental skill. Trusted curated skills cannot be mutated or deleted by the agent. See docs(op=\"read\", id=\"skills\") for the path layout.",
			InputSchema: objSchema(
