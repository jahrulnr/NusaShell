...
// Service owns memory RPC, consolidator Apply, and task-memory announcements.
type Service struct {
	deps Deps
}

// New builds a memory Service from Deps.
func New(d Deps) *Service {
	return &Service{deps: d}
}

// NewMemoryService is the consolidator constructor used by the learner job.
func NewMemoryService(records RecordStore, ops OpStore) *Service {
	return New(Deps{Records: records, Ops: ops})
}

func (s *Service) Apply(op *domain.LearningOperation) error {
	if s == nil || s.deps.Records == nil || op == nil {
	...
	case domain.OpMemoryUpsert:
		...
		if existing, gerr := s.deps.Records.Get(rec.ID); gerr == nil && existing != nil && existing.Retrievable() {
		...
		if err := s.deps.Records.Save(rec); err != nil {
			op.Status = domain.LearningOpRejected
			op.Reason = err.Error()
			s.saveOp(op)
			return err
	...
	case domain.OpMemoryMerge:
		...
		keep, err := s.deps.Records.Get(keepID)
		if err != nil {
			op.Status = domain.LearningOpRejected
			s.saveOp(op)
			return err
		}
		drop, err := s.deps.Records.Get(dropID)
		if err != nil {
			op.Status = domain.LearningOpRejected
			s.saveOp(op)
			return err
		...
		_ = s.deps.Records.Save(drop)
		if err := s.deps.Records.Save(keep); err != nil {
			op.Status = domain.LearningOpRejected
			s.saveOp(op)
			return err
		}
...
func (s *Service) saveOp(op *domain.LearningOperation) {
	if s.deps.Ops != nil && op != nil {
		_ = s.deps.Ops.Save(op)
	}
}
...
func (s *Service) Reject(op *domain.LearningOperation, reason string) {
	if op == nil {
		return
	}
	op.Status = domain.LearningOpRejected
