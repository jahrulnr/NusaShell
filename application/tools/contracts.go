...
// BuildContracts returns the frontend contract for the same interactive
...
func BuildContracts(defs []ToolInfo) contracts.ToolContractsResult {
	out := make([]contracts.ToolContractDTO, 0, len(defs))
	for _, def := range defs {
		if strings.TrimSpace(def.Name) == "" {
			continue
		}
		out = append(out, BuildContract(def))
	}
...
// BuildContract maps one advertised tool into the frontend contract DTO.
func BuildContract(def ToolInfo) contracts.ToolContractDTO {
