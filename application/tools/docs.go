...
func (s *Service) HandleList() (any, *contracts.RPCError) {
	if s.deps.Docs == nil {
		return contracts.DocsListResult{Docs: []contracts.DocDTO{}}, nil
	}
	metas := s.deps.Docs.List()
...
func (s *Service) HandleSearch(req contracts.DocsSearchRequest) (any, *contracts.RPCError) {
	if s.deps.Docs == nil {
		return contracts.DocsSearchResult{Results: []contracts.DocHit{}}, nil
	}
	limit := req.Limit
	if limit <= 0 || limit > 20 {
		limit = 10
	}
	hits := s.deps.Docs.Search(req.Query, limit)
...
func (s *Service) HandleRead(req contracts.DocReadRequest) (any, *contracts.RPCError) {
	if s.deps.Docs == nil {
		return nil, &contracts.RPCError{Code: contracts.CodeNotFound, Message: "docs store not available"}
	}
	doc, err := s.deps.Docs.Read(req.ID)
	if err != nil {
