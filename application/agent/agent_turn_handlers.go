...
func (a *Service) HandleTurnsRetry(ctx context.Context, req contracts.TurnRetryRequest) (any, *contracts.RPCError) {
	...
	turnCtx, cancel := context.WithCancel(context.WithoutCancel(ctx))
	run := &TurnRun{ID: domain.NewID(domain.IDPrefixRun), ConversationID: c.ID, MessageID: targetMsgID, Ctx: turnCtx, Cancel: cancel, ProviderID: provider.ID, Workspace: a.effectiveWorkspace(c.Workspace)}
	a.runsMu.Lock()
	a.runs[run.ID] = run
	a.runsMu.Unlock()
...
func (a *Service) AddTurnMessages(c *domain.Conversation, userMsg, asstMsg domain.Message) {
...
func (a *Service) TakeWorkspaceSwitchNotice(c *domain.Conversation) *domain.Message {
...
func (a *Service) workspaceSwitchNotice(from, to string) domain.Message {
...
func (a *Service) readWorkspaceAgentsMD(ws string) hydrationSlot {
...
func (a *Service) HandleToolStop(req contracts.ToolStopRequest) (any, *contracts.RPCError) {
...
func (a *Service) ActiveRunForConversation(convID string) *TurnRun {
	a.runsMu.Lock()
	defer a.runsMu.Unlock()
	return a.ActiveRunForConversationLocked(convID)
}

func (a *Service) ActiveRunForConversationLocked(convID string) *TurnRun {
...
func (a *Service) HealOrphanedRunningConversation(c *domain.Conversation) bool {
	if c.Status != "running" {
		return false
	}
	if a.ActiveRunForConversation(c.ID) != nil {
...
func (a *Service) RecoverOrphanedTurn(run *TurnRun) {
	defer func() {
		if r := recover(); r != nil {
...
func (a *Service) HandleTurnsActive(req contracts.ConversationIDRequest) (any, *contracts.RPCError) {
...
func (a *Service) HandleAskPendingList(req contracts.AskPendingListRequest) (any, *contracts.RPCError) {
	if a.AskQuestions == nil {
...
func (a *Service) HandleAskAnswer(req contracts.AskAnswerRequest) (any, *contracts.RPCError) {
	if a.AskQuestions == nil {
...
func (a *Service) HandleAskCancel(req contracts.AskCancelRequest) (any, *contracts.RPCError) {
	if a.AskQuestions == nil {
		return nil, &contracts.RPCError{Code: contracts.CodeNotFound, Message: "ask_question is not available"}
	}
	if req.RunID == "" || req.ToolCallID == "" {
...
func (a *Service) HandleTurnsCancelSteer(req contracts.TurnCancelSteerRequest) (any, *contracts.RPCError) {
	run := a.ActiveRunForConversation(req.ConversationID)
	if run == nil {
		return nil, &contracts.RPCError{Code: contracts.CodeConflict, Message: "no active turn for this conversation"}
	}
