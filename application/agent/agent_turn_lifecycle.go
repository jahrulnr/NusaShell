...
// is watching.
func (a *Service) EmitInteractiveTurnEvent(run *TurnRun, typ string, payload any) {
...
func (a *Service) UpdateToolResult(c *domain.Conversation, msgID, callID string, status domain.ToolCallStatus, output string, outputAttachments []domain.Attachment) *domain.Conversation {
	for i := range c.Messages {
...
func (a *Service) FailTurn(run *TurnRun, msgID string, err error) {
	if msgID == "" {
...
func (a *Service) FailStreamTurn(run *TurnRun, msgID, model string, round StreamedTurnRound, err error) {
	if msgID == "" {
		msgID = run.CurrentMessageID()
	}
	a.log("error", "agent", "turn failed: %s: %v", run.ID, err)
...
func (a *Service) InterruptTurn(run *TurnRun, msgID string, round StreamedTurnRound, usage ChatUsage, contextTokens int, model string) {
