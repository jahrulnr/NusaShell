...
func (a *Service) PersistTurnRound(conversationID, messageID, model string, round StreamedTurnRound) error {
...
func (a *Service) publishRoundDelta(runID, messageID string, round int, kind, toolCallID, name, text string) {
...
func (a *Service) publishRoundToolStart(runID, messageID string, round int, toolCallID, name, args string, presentation *contracts.ToolPresentationDTO) {
	if a.RoundStreams != nil {
...
func (a *Service) AppendTurnAssistant(conversationID string) (*domain.Conversation, string, error) {
	repo, err := a.loadRepo(conversationID)
	if err != nil {
		return nil, "", err
	}
...
func (a *Service) finishTurn(run *TurnRun, messageID, model string, usage ChatUsage, contextTokens, autoContinueIndex int) error {
