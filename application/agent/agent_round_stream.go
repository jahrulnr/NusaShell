...
func (a *Service) learnFromStreamError(run *TurnRun, model string, err error, caps *ModelCapabilities) bool {
...
func (a *Service) prepareStreamRetry(run *TurnRun, messageID string, retry int, delay time.Duration, err error) {
...
func (a *Service) StreamTurnRoundOnce(run *TurnRun, adapter ProviderContext, conversation *domain.Conversation, messageID, model, effort string, tools []ToolDef, settings domain.Settings, continuation bool, partial *StreamedTurnRound, maxTokens int, promptCache *PromptCachePolicy, caps ModelCapabilities, round int) (StreamedTurnRound, error) {
	...
	}, func(event provider.ToolUseDelta) {
