...
func (a *Service) ExecuteTurnTools(run *TurnRun, messageID string, toolCalls []domain.ToolCall, caps ModelCapabilities, settings domain.Settings, round int) error {
...
func (a *Service) RunOneTool(run *TurnRun, messageID string, toolCall domain.ToolCall, caps ModelCapabilities, settings domain.Settings, round int) ToolExecResult {
	...
	case learnerResultToolName:
		if !isLearnerKind(run.ToolKind) {
			...
			output, err = a.acknowledgeLearnerResult(toolCall.Args)
		}
	case "read_media":
...
func (a *Service) emitToolCompleted(run *TurnRun, toolCall domain.ToolCall, res toolExecResult) {
...
func (a *Service) emitLearningMutationEvents(toolName string, status domain.ToolCallStatus) {
