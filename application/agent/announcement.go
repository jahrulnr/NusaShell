...
// instead of N cards.
func (a *Service) PendingAnnouncementsMessage(items []domain.PendingAnnouncement) domain.Message {
...
func (a *Service) DrainAnnouncements(run *TurnRun) (bool, error) {
	if run == nil {
		return false, nil
	}
...
func (a *Service) PublishAnnouncement(convID string, ev Announcement) {
	if convID == "" || ev.Type == "" {
		return
	}
...
func (a *Service) PublishAnnouncementToAll(ev Announcement, skipConvID string) {
...
func (a *Service) announcementLock(convID string) *sync.Mutex {
