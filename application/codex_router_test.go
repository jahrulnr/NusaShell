...
func TestIsRateLimitError(t *testing.T) {
	...
	err := &domain.ProviderError{StatusCode: 429, Err: errors.New("rate limited")}
	if !isRateLimitError(err) {
		t.Fatal("429 should be rate limit")
	}
	err2 := &domain.ProviderError{StatusCode: 500, Err: errors.New("server error")}
...
func TestRateLimitCooldown(t *testing.T) {
	// With Retry-After
	err := &domain.ProviderError{StatusCode: 429, RetryAfter: 2 * time.Minute, Err: errors.New("rate limited")}
	...
	err2 := &domain.ProviderError{StatusCode: 429, Err: errors.New("rate limited")}
