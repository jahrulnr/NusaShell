...
// ErrImmutable is returned by Repository.Save when message IDs would be
// inserted, reordered, or deleted. Append with Add, or start a new epoch
...
func NewConversation(store Store, title string) *Repository {
...
// Bind wraps an already-loaded conversation so Add/Save go through the
// repository. Callers must bind before mutating messages so Save can
// compare against the ID prefix at load time.
func Bind(store Store, c *domain.Conversation) *Repository {
	r := &Repository{store: store, inner: c}
	r.rememberPersistedIDs()
	return r
}

func (r *Repository) rememberPersistedIDs() {
...
func (r *Repository) Conversation() *domain.Conversation {
	if r == nil {
		return nil
	}
	return r.inner
...
func (r *Repository) GetAll() []domain.Message {
	n := 0
	if r != nil && r.inner != nil {
...
func (r *Repository) GetFrom(start, end int) []domain.Message {
	if r == nil || r.inner == nil {
...
func (r *Repository) GetById(id string) error {
...
func (r *Repository) Add(role domain.MessageRole, args ...any) error {
	if r == nil || r.inner == nil {
...
func (r *Repository) ResetTranscript() {
	if r == nil || r.inner == nil {
		return
	}
