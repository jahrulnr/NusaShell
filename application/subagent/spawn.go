...
// SpawnSubagents is the `subagent` tool implementation. conversationID and
...
func (s *Service) SpawnSubagents(ctx context.Context, conversationID, toolCallID string, argsJSON []byte) (string, error) {
	if s.deps.Runtime == nil || s.deps.Agents == nil {
	...
	if strings.TrimSpace(args.AgentID) != "" {
		agent, err = s.deps.Agents.Get(args.AgentID)
		if err != nil {
