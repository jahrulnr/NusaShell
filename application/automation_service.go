...
// Automation is the application facade for Automation workflows, scheduling, and execution.
...
func workflowDTO(w *domain.WorkflowDefinition, avail string, reason string) contracts.WorkflowDTO {
...
func runDTO(r *domain.WorkflowRun) contracts.RunDTO {
	jobs := make([]contracts.JobRunDTO, 0, len(r.Jobs))
	for _, j := range r.Jobs {
		steps := make([]contracts.StepRunDTO, 0, len(j.Steps))
		for _, s := range j.Steps {
			st := contracts.StepRunDTO{ID: s.ID, Name: s.Name, Status: string(s.Status), ExitCode: s.ExitCode, Error: s.Error, Output: s.Output}
			if !s.StartedAt.IsZero() {
				st.StartedAt = clock.NewTime(s.StartedAt).Format(time.RFC3339)
			}
			steps = append(steps, st)
		}
		jd := contracts.JobRunDTO{ID: j.JobID, Name: j.Name, Status: string(j.Status), ExitCode: j.ExitCode, Error: j.Error, Steps: steps}
	...
	dto := contracts.RunDTO{
		...
		Summary: contracts.RunSummaryDTO{
