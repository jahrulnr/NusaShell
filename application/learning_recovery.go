	...
	for _, job := range a.LearningJobs.List() {
		...
		case domain.LearningJobRunning:
			...
			if err := a.LearningJobs.Save(job); err == nil {
				a.log("warn", "learning", "recovered stale running job: id=%s started=%s", job.ID, job.StartedAt.Format(time.RFC3339))
			}
		case domain.LearningJobQueued:
			...
			job.FinishedAt = &now
			if err := a.LearningJobs.Save(job); err == nil {
				a.log("warn", "learning", "expired abandoned queued job: id=%s created=%s", job.ID, job.CreatedAt.Format(time.RFC3339))
			}
		}
	}
}
