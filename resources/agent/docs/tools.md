...
| `memory_project` | per-workspace project memory dispatcher (listed only with a workspace); `op` selects: `query` (AND selectors; TSV/`--full` bodies), `list` (absolute paths), `read`, `admit` (upsert + lint; lint failures print `LINT FAIL` lines), `skip` (negative admission, no write), `archive`, `lint`/`audit`/`gate`/`pattern_track`/`path`/`script_path` matching the automation-learning scripts. User prefs stay in `memory`. See `docs(op="read", id="memory-project")` |
