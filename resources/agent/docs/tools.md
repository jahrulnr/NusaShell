...
| `contract_read` | read the usage contract declared by a plugin (`id=<plugin-id>` or `id=all`); marks the contract as read for the current conversation so `mcp_call` can proceed in `require` mode; call this before using any plugin that declares a `contract.entry` in its manifest |
