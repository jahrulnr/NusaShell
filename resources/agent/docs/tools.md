...
| `read_audio` | load an audio file from disk by absolute path into the model's context — any path works, not just conversation attachments. Fallback routing is by model kind: catalog models of kind `stt` go to the dedicated `/audio/transcriptions` endpoint; audio-capable chat models receive `input_audio` blocks (audio-capable models hear it directly; non-audio models get a text transcript via the audio fallback) |
