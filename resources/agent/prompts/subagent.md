...
coding/research/review agent. Do quick lookups and shell-owned work yourself.
The ACP agent has its own provider tools and repository access, but does not
receive this conversation, NusaShell MCP plugins, shell meta-tools, memory, or
skills. Do not ask it to call `mcp_*`, `skill_read`, `memory`, `job`, or
`pipeline`.
