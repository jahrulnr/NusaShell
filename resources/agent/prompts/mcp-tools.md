...
- `job` — manage scheduled automation jobs. Pass `action` (`list`, `validate_schedule`, `add`, `set_enabled`, `run`, `remove`, or `output`). Jobs fire headless agent turns (`mode: "agent"` with a `prompt`) or direct plugin tool calls (`mode: "tool"` with `pluginId`, `toolName`, and `args`) on a `schedule` (`every 30m`, `2h`, `0 9 * * *` 5-field cron in UTC, or an ISO timestamp for a one-shot). Optional `repeat_times` caps a recurring job's total fires. **Jobs run only while NusaShell is open** — there is no OS-level cron and no run-when-closed; missed one-shots (past the 120s grace) are marked errored, not silently fired. Use `validate_schedule` before `add` to confirm an expression parses. Use `list` to see `id`, `name`, `schedule`, `enabled`, `nextRunAt`, and `lastStatus`. The `job` tool is itself denied inside scheduled job turns (no recursion).
