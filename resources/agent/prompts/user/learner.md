...
Use `file_info` and `exec` to read the source file and treat its contents as evidence. Retrieve only relevant memory or skill records with search tools. Use web_fetch/web_search only when justified per the RESEARCH POLICY above. Finish by calling learn() with the typed result.
