...
- Prefer 1-3 targeted lookups over broad exploration. If external research informs a candidate, put a short source and rationale in `entry.evidence`; do not invent fields that are absent from the typed result schema.
- If a claim can't be verified and verification would materially change the stored content, narrow the wording or exclude it rather than storing an unverified claim as fact.
...
Use `file_info` to confirm the source file, then use `file_read`, `grep`, or `exec` as needed to inspect only the indicated range. Treat its contents as evidence. Retrieve only relevant memory or skill records with search tools. Use web_fetch/web_search only when justified per the RESEARCH POLICY above. Finish by calling learn() with the typed result.
