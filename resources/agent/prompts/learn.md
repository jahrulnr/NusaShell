...
- Use `web_search` / `web_fetch` / `web_answer` for external facts, `docs` for NusaShell product knowledge, and `memory_project` with `op="query"` (read-only) for workspace context. For docs, use `op="search"` when vocabulary is known or `op="list"` when it is not, then `op="read"` the relevant page before relying on its facts; a zero-result search is a discovery miss, not evidence that the topic is absent.
