...
| Agent observers / `notify:` sink | Supported | Side-effect only; trusted workflows; never rely on it in logic | Per-event FIFO observers (run/step/tool_call pre+post w/ CallID, reasoning/text per round); streams to `internal_send_progress`/`admin.send_progress`; best-effort, never blocks/fails run |
