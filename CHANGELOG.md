...
- **Codex opaque compaction now preserves transcript order.** The active epoch
  keeps its retained user, assistant, and tool messages in chronological order
  after compaction, while the Codex provider adapter still builds the wire
  prefix required by the upstream API. This prevents user messages from
  stacking together after a compaction checkpoint.
