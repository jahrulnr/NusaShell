...
async function loadListing() {
  ...
  try {
    const data = await callTool("files_list", { path: toolPath(state.currentPath) });
    state.items = expectArray(data, "items", "files_list"); state.selectedItem = null;
...
async function loadTree() { try { const data = await callTool("files_tree", { path: "/", depth: 3 }); state.tree = expectArray(data, "tree", "files_tree"); renderTree(); } catch { elements["tree-container"].innerHTML = '<p class="tree-empty">Folders are unavailable</p>'; } }
...
async function openPreview(item) {
  ...
  try {
    const info = await callTool("files_info", { path: toolPath(path) });
    renderPreviewMeta(info);
    if (item.type === "text") { const result = await callTool("files_read", { path: toolPath(path) }); elements["preview-content"].textContent = result.content || "This file is empty."; }
...
async function handleNewFile() { const value = await openModal({ title:"Create file", description:"A new text file will be created in the current folder.", fieldLabel:"File name", showContent:true, submitLabel:"Create file" }); if (!value) return; try { await callTool("files_write", { path: toolPath(joinPath(state.currentPath, value.name)), content:value.content }); toast("File created", "success"); refresh(); } catch (error) { toast(error.message, "error"); } }
async function handleNewFolder() { const value = await openModal({ title:"Create folder", description:"Folders can be empty and can contain other folders.", fieldLabel:"Folder name", submitLabel:"Create folder" }); if (!value) return; try { await callTool("files_mkdir", { path: toolPath(joinPath(state.currentPath, value.name)) }); toast("Folder created", "success"); refresh(); } catch (error) { toast(error.message, "error"); } }
async function handleRename() { if (!state.selectedItem) return; const item = state.selectedItem; const value = await openModal({ title:"Rename", description:"The item stays in the same folder.", fieldLabel:"New name", defaultValue:item.name, submitLabel:"Rename" }); if (!value || value.name === item.name) return; try { await callTool("files_move", { source:toolPath(itemPath(item)), destination:toolPath(joinPath(state.currentPath, value.name)) }); toast("Renamed", "success"); refresh(); } catch (error) { toast(error.message, "error"); } }
async function handleDelete() { if (!state.selectedItem) return; const item = state.selectedItem; const confirmed = await openModal({ title:"Delete item?", description:"Please review this destructive action.", confirm:item.name, submitLabel:"Delete", danger:true }); if (!confirmed) return; try { await callTool("files_delete", { path:toolPath(itemPath(item)), recursive:item.isDir }); toast("Deleted", "success"); refresh(); } catch (error) { toast(error.message, "error"); } }
async function handleSearch(query) { const clean = query.trim(); if (!clean) { state.searchQuery=""; state.searchResults=null; renderListing(); return; } state.searchQuery=clean; try { const result=await callTool("files_search", { path:toolPath(state.currentPath), pattern:clean.includes("*") || clean.includes("?") ? clean : `*${clean}*` }); state.searchResults=expectArray(result, "results", "files_search"); state.selectedItem=null; renderListing(); updateActions(); } catch(error) { toast(error.message,"error"); } }
