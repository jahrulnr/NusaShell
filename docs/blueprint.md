...
- Jobs is a shell-owned surface for scheduled automation. Each job fires either a headless agent turn or a direct plugin tool call on a once/interval/cron schedule. Jobs run only while NusaShell is open; a one-shot missed while the app was closed is marked errored, not silently fired. The view lists all jobs with schedule, mode, repeat progress, and next run time; each row offers Run, Pause/Resume, Output, and Remove. A modal form creates new jobs with live schedule validation. `job.completed`/`job.failed` events trigger a desktop toast and list refresh. See `docs/architecture/job-automation.md` for the full design.
