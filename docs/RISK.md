...
- **`npx` argument swap.** A manifest that runs `npx -y @someplugin` can have
  ...
  shell does **not** maintain a hard allowlist of args/env for MVP (security is
  deferred per the architecture locks). Mitigations today:
  ...
  - Future: an allowlist of permitted arg/env mutations, signed manifests, and
    user approval UX before applying agent-supplied overrides.
