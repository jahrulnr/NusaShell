...
Supported provider kinds are `messages`, `responses`, `chat`, and `codex` (OpenRouter hosts are auto-detected by Base URL). There is intentionally no fallback layering: stream-unsupported, empty-stream, and image-strip retries were removed — errors surface explicitly to the retry loop. Codex remote v2 compaction is a separate streaming `/responses` request with a final `compaction_trigger` item; it is not OpenAI `context_management`.
...
- A release commit bumps `VERSION` in the same commit that publishes its `CHANGELOG.md` section; changes that do not ship keep their entry under `## [Unreleased]`. `scripts/version.test.mjs` fails when the newest CHANGELOG section is neither `[Unreleased]` nor the current `VERSION`, so a forgotten bump fails CI instead of silently skipping that stream's release.
