// Package plugin manages installed plugins end to end. The Store keeps
// each plugin under <datadir>/plugins/<id>/ (manifest.json plus mcp/ and
// ui/ directories), the icon helpers resolve manifest icons for display,
// the Installer fetches catalog/GitHub/ZIP sources into the store, and the
// Manager runs plugin MCP servers and routes tool calls from plugin UIs.
package plugin
...
// NewStore creates a plugin store rooted at root. The directory is created
// if it does not exist.
func NewStore(root string) (*Store, error) {
	if err := os.MkdirAll(root, 0o755); err != nil {
		return nil, fmt.Errorf("plugin: mkdir %s: %w", root, err)
	}
...
func (s *Store) List() ([]*domain.Plugin, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("plugin: read %s: %w", s.root, err)
	}
...
func (s *Store) safePluginDir(id string) (string, error) {
	...
	if !strings.HasPrefix(dir, s.root+string(filepath.Separator)) && dir != s.root {
		return "", fmt.Errorf("plugin: id %q escapes plugins root", id)
	}
	return dir, nil
}
...
func (s *Store) Install(sourceDir string) (*domain.Plugin, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("plugin: resolve source %s: %w", sourceDir, err)
	}
	rootPath, err := filepath.EvalSymlinks(s.root)
	if err != nil {
		return nil, fmt.Errorf("plugin: resolve root %s: %w", s.root, err)
	}
	if rel, err := filepath.Rel(rootPath, sourcePath); err == nil && (rel == "." || (rel != ".." && !strings.HasPrefix(rel, ".."+string(filepath.Separator)))) {
		return nil, fmt.Errorf("plugin: source directory must be outside the installed plugins root: %s", sourceDir)
	}
	...
	if _, statErr := os.Stat(destDir); statErr == nil {
		if err := os.RemoveAll(destDir); err != nil {
			return nil, fmt.Errorf("plugin: remove old %s: %w", destDir, err)
		}
	} else if !os.IsNotExist(statErr) {
		return nil, fmt.Errorf("plugin: stat destination %s: %w", destDir, statErr)
	}
	if err := copyDir(sourceDir, destDir); err != nil {
		return nil, fmt.Errorf("plugin: copy %s → %s: %w", sourceDir, destDir, err)
	}
...
func (s *Store) Save(p *domain.Plugin) error {
	if p == nil || p.Manifest.ID == "" {
		return fmt.Errorf("plugin: cannot save plugin without id")
	}
...
func (s *Store) loadManifest(dir string) (*domain.PluginManifest, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("plugin: read manifest.json in %s: %w", dir, err)
	}
	var m domain.PluginManifest
	if err := json.Unmarshal(data, &m); err != nil {
		return nil, fmt.Errorf("plugin: parse manifest.json in %s: %w", dir, err)
	}
...
// before the /plugins endpoint returns them (file:// is blocked by the
// browser from an http origin).
func (s *Store) ToUIEntry(p *domain.Plugin) contracts.PluginUIEntryDTO {
	return contracts.PluginUIEntryDTO{
		ID:          p.Manifest.ID,
		Name:        p.Manifest.Name,
		Version:     p.Manifest.Version,
		Icon:        ResolveLocal(p.Manifest.Icon, p.InstallPath),
