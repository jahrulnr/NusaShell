...
// Manager owns the plugin store + MCP connection manager and the lifecycle
// of installed plugins: starting/stopping their MCP servers and routing tool
// calls from plugin UIs to the MCP server. It is safe for concurrent use.
type Manager struct {
	store *Store
	mcp   *mcpclient.Manager
}

// NewManager creates a runtime manager backed by the given store and MCP
// manager. It wraps mcpclient.Manager, which operates directly on a plugin's
// manifest MCP config, so the MCP connection logic stays in one place.
func NewManager(store *Store, mcp *mcpclient.Manager) *Manager {
	return &Manager{
		store: store,
		mcp:   mcp,
	}
