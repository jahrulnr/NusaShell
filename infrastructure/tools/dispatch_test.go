...
func TestAllAdvertisedFamilyOpsRoute(t *testing.T) {
	...
	cases := []struct{ name, args string }{
		...
		{"memory_project", `{"op":"audit"}`},
		{"memory_project", `{"op":"gate","reason":"nothing durable"}`},
		{"memory_project", `{"op":"pattern_track","kind":"debug"}`},
		{"memory_project", `{"op":"path","kind":"debug"}`},
		{"memory_project", `{"op":"script_path","name":"trace-turn.sh"}`},
