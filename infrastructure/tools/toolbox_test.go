...
func TestSkillSearch(t *testing.T) {
	...
	if !strings.Contains(out, `"path":"/home/user/.local/nusashell/skills/git-helper"`) {
		t.Errorf("skill search must expose the absolute path so the model can load the skill, got: %s", out)
	}
	if !strings.Contains(out, `"bundled":true`) {
		t.Errorf("skill search must expose the bundled flag so the model knows subfiles exist, got: %s", out)
	}
...
func TestTodoToolResultDoesNotEchoItemsOrBrief(t *testing.T) {
	dir := t.TempDir()
	store := jsonstore.NewTodoStore(filepath.Join(dir, "todos.json"), dir)
