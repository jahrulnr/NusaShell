...
func (p *Provider) convertResponse(resp *chatResponse, req *core.Request) (*core.Response, error) {
	out := &core.Response{
		Provider: p.Name(),
		Model:    req.Model,
	...
	if reasoning != "" || len(extra) > 0 {
		out.Blocks = append(out.Blocks, core.ReasoningBlock{Text: reasoning, Extra: extra})
	}
	...
	if choice.Message.Refusal != "" {
		out.Blocks = append(out.Blocks, core.TextBlock{Text: choice.Message.Refusal})
	}
	if out.Refusal != "" {
...
func convertUsage(u usage, spec Spec, provider, model string) core.Usage {
	out := core.Usage{
...
func contentBlocks(raw json.RawMessage) ([]core.Block, string, error) {
