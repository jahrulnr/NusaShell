...
// AudioBlock is an inline audio input (input_audio parts in the OpenAI
// wire formats, base64 source in the Anthropic wire format). Format is the
// normalized audio container ("mp3", "wav", "ogg", "webm", "flac"); empty
...
// VideoBlock is a video input (video_url parts in the OpenAI/OpenRouter
// wire formats, base64 source in the Anthropic wire format). URL accepts
// public URLs and base64 data URLs.
...
type Request struct {
	...
	TopP        *float64
	TopK        *int
	// FrequencyPenalty and PresencePenalty are OpenAI-wire sampling
	// parameters, carried here so OpenAI-compatible providers can emit
	// them without re-deriving from provider options.
