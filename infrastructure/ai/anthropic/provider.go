...
func (p *Provider) Chat(ctx context.Context, req *core.Request) (*core.Response, error) {
	...
	if err := json.Unmarshal(data, &parsed); err != nil {
		return nil, core.NewProviderErrorWithCause(p.Name(), core.ErrorTypeProvider, "anthropic: decode response", err)
	}
...
// a version or a full endpoint it is honored verbatim; a bare root gets the
// Anthropic-compatible convention suffix /v1/messages — this is how
...
func cacheControl(cache *core.CacheControl) (*anthropicCacheControl, error) {
	if cache == nil {
		return nil, nil
	}
	cc := &anthropicCacheControl{Type: cache.Type}
	if cc.Type == "" {
		cc.Type = core.CacheTypeEphemeral
	}
	if cc.Type != core.CacheTypeEphemeral {
		return nil, fmt.Errorf("anthropic: unsupported cache type %q", cache.Type)
	}
	if cache.TTL != "" && cache.TTL != core.CacheTTL5m {
		if cache.TTL != core.CacheTTL1h {
...
func imageSource(block core.ImageBlock) (*anthropicImageSource, error) {
	switch {
	case block.URL != "":
