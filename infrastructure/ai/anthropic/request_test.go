	...
	req := &core.Request{
		...
		Messages: []core.Message{
			...
			core.User(
				core.TextBlock{
					Text: "Use the tool.",
					Cache: &core.CacheControl{
						Type: core.CacheTypeEphemeral,
						TTL:  core.CacheTTL1h,
					},
				},
			),
			core.Assistant(
				core.ReasoningBlock{Text: "I should call the tool.", Signature: "sig-thinking"},
				core.ToolUseBlock{ID: "toolu_1", Name: "lookup", Arguments: core.MustJSONRaw(map[string]any{"q": "x"})},
			),
			core.ToolResult("toolu_1",
				core.Text("result text"),
				core.ToolReferenceBlock{ToolName: "lookup"},
			),
...
func TestStructuredCapabilityIsModelDependent(t *testing.T) {
	...
	if got := provider.Capabilities("claude-custom").Structured.JSONSchema; got != core.SupportUnknown {
...
func TestThinkingCapabilitiesExposeStableBaseline(t *testing.T) {
	...
	if caps.Supported != core.SupportYes || caps.Disable != core.SupportUnknown {
...
func TestBuildRequestUsesRedactedThinkingData(t *testing.T) {
	...
	maxTokens := 4096
	wire, _, err := provider.buildRequest(&core.Request{
		Model:     "claude",
		MaxTokens: &maxTokens,
		Messages: []core.Message{
			core.Assistant(core.ReasoningBlock{Redacted: []byte("opaque")}),
		},
	}, false)
	if err != nil {
		t.Fatalf("buildRequest returned error: %v", err)
...
func TestBuildRequestMapsMaxThinkingEffort(t *testing.T) {
	...
	wire, _, err := provider.buildRequest(&core.Request{
		...
		MaxTokens: &maxTokens,
		Messages:  []core.Message{core.UserText("hi")},
		Thinking:  &core.Thinking{Mode: core.ThinkingEnabled, Effort: "max"},
	}, false)
	if err != nil {
		t.Fatalf("buildRequest returned error: %v", err)
	}
...
func TestBuildRequestRejectsThinkingBudget(t *testing.T) {
	...
	_, _, err = provider.buildRequest(&core.Request{
		...
		MaxTokens: &maxTokens,
		Messages:  []core.Message{core.UserText("hi")},
		Thinking:  &core.Thinking{Mode: core.ThinkingEnabled, BudgetTokens: &budget},
	}, false)
...
func TestBuildRequestValidatesThinkingTopP(t *testing.T) {
	...
	topP = 0.99
	if _, _, err := provider.buildRequest(&core.Request{
		Model:     "claude-sonnet-5",
		MaxTokens: &maxTokens,
		TopP:      &topP,
		Messages:  []core.Message{core.UserText("hi")},
...
func TestResponseBlocksRoundTripBackIntoAssistantMessage(t *testing.T) {
	...
	wire, _, err := provider.buildRequest(&core.Request{
		...
		Messages: []core.Message{
			...
			core.ToolResultText("toolu_1", "result"),
		},
	}, false)
	if err != nil {
		t.Fatalf("buildRequest returned error: %v", err)
...
func TestBuildRequestAdaptiveThinkingOnModernModels(t *testing.T) {
	...
	maxTokens := 4096
	wire, warnings, err := provider.buildRequest(&core.Request{
...
func TestBuildRequestMapsJSONSchemaResponseFormat(t *testing.T) {
	...
	maxTokens := 1024
	schema, err := core.SchemaFrom(map[string]any{
		"type":                 "object",
	...
	wire, warnings, err := provider.buildRequest(&core.Request{
		...
		ResponseFormat: &core.ResponseFormat{
			...
			JSONSchema: &core.JSONSchema{Name: "person", Schema: schema},
		},
	}, false)
	if err != nil {
		t.Fatalf("buildRequest returned error: %v", err)
...
func TestBuildRequestKeepsEmptyToolUseInput(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			call: core.ToolUseBlock{ID: "toolu_empty", Name: "novel_context"},
		},
		{
			...
			call: core.ToolUseBlock{
				ID:        "toolu_empty",
				Name:      "novel_context",
				Arguments: core.MustJSONRaw(map[string]any{}),
			},
		},
	}
...
func TestBuildRequestMergesConsecutiveToolResultMessages(t *testing.T) {
	...
	wire, _, err := provider.buildRequest(&core.Request{
		...
		Messages: []core.Message{
			core.UserText("hi"),
			core.Assistant(
				core.ToolUseBlock{ID: "toolu_1", Name: "a", Arguments: core.MustJSONRaw(map[string]any{})},
				core.ToolUseBlock{ID: "toolu_2", Name: "b", Arguments: core.MustJSONRaw(map[string]any{})},
			),
			core.ToolResultText("toolu_1", "one"),
			core.ToolResultText("toolu_2", "two"),
		},
	}, false)
	if err != nil {
		t.Fatalf("buildRequest returned error: %v", err)
