...
func TestLegacyCompactionInputWireShape(t *testing.T) {
	// The legacy unary endpoint DTO is modeled for fidelity: minimal fields,
	// optional fields omitted when absent, no store/stream controls.
	...
	for _, key := range []string{"model", "input", "parallel_tool_calls"} {
		if _, ok := probe[key]; !ok {
			t.Fatalf("legacy body missing %q: %s", key, body)
		}
	}
	if _, ok := probe["store"]; ok {
		t.Fatalf("legacy body unexpectedly carries store: %s", body)
	}
	if LegacyCompactPath != "responses/compact" {
		t.Fatalf("LegacyCompactPath = %q", LegacyCompactPath)
	}
}

func TestCompactResponseDecodesOpaqueOutput(t *testing.T) {
	var resp CompactResponse
	if err := json.Unmarshal([]byte(`{"output":[{"id":"cmp_1","type":"compaction","encrypted_content":"ENC-COMPACT"}]}`), &resp); err != nil {
		t.Fatalf("unmarshal: %v", err)
	}
	if len(resp.Output) != 1 || resp.Output[0].EncryptedContent != "ENC-COMPACT" {
		t.Fatalf("output = %+v, want one compaction item", resp.Output)
	}
}
