...
func (p *Provider) buildRequest(ctx context.Context, req *core.Request) (*ResponsesAPIRequest, bool, error) {
	...
	if blob, ok := stringOption(req.ProviderOptions, "compaction_items"); ok {
		input, err = codexHistoryWithCompaction(input, blob)
		if err != nil {
			return nil, false, err
		}
	}
	...
	reasoning := codexReasoning(req.Thinking)
...
func codexHistoryWithCompaction(input []ResponseItem, blob string) ([]ResponseItem, error) {
	...
	return BuildRetainedHistory(input, compaction), nil
}
...
func requestInput(messages []core.Message) (string, []ResponseItem, error) {
	...
	for index, message := range messages {
		...
		items, err := messageItems(message)
		if err != nil {
...
func messageItems(message core.Message) ([]ResponseItem, error) {
	switch message.Role {
	case core.RoleUser:
		...
		return []ResponseItem{{Type: ItemTypeMessage, Role: RoleUser, Content: content}}, nil
	...
	case core.RoleTool:
		...
		for _, block := range message.Blocks {
			...
			text, err := blocksText(result.Content)
			if err != nil {
				return nil, err
			}
...
func codexReasoning(thinking *core.Thinking) *Reasoning {
