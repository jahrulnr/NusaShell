...
func convertResponse(resp *chatResponse, req *core.Request) (*core.Response, error) {
	if resp == nil {
	...
	out.FinishReason = core.NormalizeFinishReason(choice.FinishReason)
	out.FinishReasonRaw = choice.FinishReason
	if reasoning := extractReasoning(choice); reasoning != "" && thinkingEnabled(req) {
		out.Blocks = append(out.Blocks, core.ReasoningBlock{Text: reasoning, Summary: choice.ReasoningSummary != nil})
	}
	...
	if choice.Message.Refusal != "" {
		...
		out.Blocks = append(out.Blocks, core.Text(choice.Message.Refusal))
...
func convertUsage(u *usage, model string) core.Usage {
	if u == nil {
		return core.Usage{}
	}
	out := core.Usage{
...
func convertContent(raw json.RawMessage) ([]core.Block, error) {
	...
	if err := json.Unmarshal(raw, &text); err == nil {
		...
		return []core.Block{core.TextBlock{Text: text}}, nil
	}
	...
	for _, part := range parts {
		...
		case "text":
			...
			if text != "" {
				...
				blocks = append(blocks, core.TextBlock{Text: text, Annotations: annotations, Logprobs: logprobs})
			}
		case "image_url":
...
func annotations(part map[string]any) ([]core.Annotation, error) {
...
func thinkingEnabled(req *core.Request) bool {
	return req == nil || req.Thinking == nil || req.Thinking.Mode != core.ThinkingDisabled
}
