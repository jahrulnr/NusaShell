...
func (p *Provider) Chat(ctx context.Context, req *core.Request) (*core.Response, error) {
	...
	if err := json.Unmarshal(data, &parsed); err != nil {
		return nil, core.NewProviderErrorWithCause(p.Name(), core.ErrorTypeProvider, "openai: decode response", err)
	}
...
func (p *Provider) ListModels(ctx context.Context) ([]core.ModelInfo, error) {
	...
	if err := json.NewDecoder(resp.Body).Decode(&payload); err != nil {
		return nil, core.NewProviderErrorWithCause(p.Name(), core.ErrorTypeProvider, "openai: decode models response", err)
	}
