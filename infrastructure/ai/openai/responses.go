...
func responsesRequestFromChat(req *core.Request) *ResponsesRequest {
	if req == nil {
		return nil
	}
	out := &ResponsesRequest{
		...
		Tools:              append([]core.Tool(nil), req.Tools...),
...
func (p *Provider) Responses(ctx context.Context, req *ResponsesRequest) (*core.Response, error) {
	...
	if err != nil {
		return nil, core.NewProviderErrorWithCause(p.Name(), core.ErrorTypeInternal, "openai: marshal responses request", err)
	}
	...
	if err != nil {
		return nil, core.NewNetworkError(p.Name(), "responses request failed", err)
	}
	...
	if err != nil {
		return nil, core.NewNetworkError(p.Name(), "read responses response failed", err)
	}
	var parsed responsesResponse
	if err := json.Unmarshal(data, &parsed); err != nil {
		return nil, core.NewProviderErrorWithCause(p.Name(), core.ErrorTypeProvider, "openai: decode responses response", err)
	}
	if parsed.Error != nil {
		return nil, core.NewProviderError(p.Name(), core.ErrorTypeProvider, fmt.Sprintf("openai: responses error: [%s] %s", parsed.Error.Code, parsed.Error.Message))
	}
...
func (p *Provider) ResponsesStream(ctx context.Context, req *ResponsesRequest) (core.Stream, error) {
	...
	body, err := json.Marshal(wire)
	if err != nil {
		return nil, core.NewProviderErrorWithCause(p.Name(), core.ErrorTypeInternal, "openai: marshal responses stream request", err)
	}
	streamCtx := ctx
	...
	if err != nil {
		...
		return nil, core.NewNetworkError(p.Name(), "responses stream request failed", err)
	}
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		data, _ := io.ReadAll(resp.Body)
		resp.Body.Close()
	...
	stream := core.Stream(newResponsesStream(resp, req.Model))
	return core.WithStreamIdleWatchdog(stream, cancel, p.cfg.StreamIdleTimeout, p.Name()), nil
}
...
func responsesInstructions(messages []core.Message) (string, []core.Message, error) {
	var parts []string
...
func blocksHaveCacheBreakpoint(blocks []core.Block) bool {
...
func responsesInputString(messages []core.Message) (string, bool) {
	if len(messages) != 1 || messages[0].Role != core.RoleUser {
		return "", false
	}
...
func textOnlyBlocks(blocks []core.Block) (string, error) {
...
func (p *Provider) responsesText(req *ResponsesRequest) (*responsesText, error) {
	...
	if req.ResponseFormat.Type == core.ResponseFormatJSONSchema {
...
func responsesReasoningConfig(req *ResponsesRequest) (*responsesReasoning, error) {
	...
	if req.ReasoningEffort == "" && req.ReasoningSummary == "" && req.ReasoningMode == "" && req.ReasoningContext == "" && (req.Thinking == nil || req.Thinking.Mode == core.ThinkingUnspecified) {
		return nil, nil
	}
	...
	if out.Effort != "" && req.Thinking != nil && req.Thinking.Mode == core.ThinkingDisabled {
		return nil, fmt.Errorf("openai: thinking disabled conflicts with reasoning_effort")
	}
	if req.Thinking != nil {
		switch req.Thinking.Mode {
		case core.ThinkingDisabled:
			out.Effort = "none"
		case core.ThinkingEnabled:
			if out.Effort == "" {
...
func responsesTools(tools []core.Tool, hosted []ResponsesTool) ([]responsesToolWire, error) {
...
func convertResponsesResponse(resp *responsesResponse, fallbackModel string) (*core.Response, error) {
	if resp == nil {
	...
	out := &core.Response{
		Model:           resp.Model,
		Provider:        "openai",
		FinishReason:    core.NormalizeFinishReason(resp.Status),
	...
	if resp.IncompleteDetails != nil && resp.IncompleteDetails.Reason != "" {
		...
		out.FinishReason = core.NormalizeFinishReason(resp.IncompleteDetails.Reason)
	}
	if out.Model == "" {
	...
	if len(out.Blocks) == 0 && resp.OutputText != "" {
		out.Blocks = append(out.Blocks, core.Text(resp.OutputText))
	}
	return out, nil
}
...
func (s *responsesStream) Next() (core.Event, error) {
	if len(s.pending) > 0 {
		event := s.pending[0]
	...
	for s.scanner.Scan() {
		...
		if data == "[DONE]" {
			s.done = true
			return core.DoneEvent{Provider: "openai", Model: s.model}, nil
		}
...
func (s *responsesStream) events(name string, raw json.RawMessage) ([]core.Event, error) {
	...
	case "response.output_text.delta":
		...
		return []core.Event{core.ContentDelta{Text: delta.Delta, OutputIndex: delta.OutputIndex, ContentIndex: delta.ContentIndex}}, nil
	case "response.refusal.delta":
		...
		return []core.Event{core.RefusalDelta{Text: delta.Delta, OutputIndex: delta.OutputIndex, ContentIndex: delta.ContentIndex}}, nil
	...
	case "response.function_call_arguments.done":
		...
		if !s.toolSeen[done.ItemID] && done.Arguments != "" {
			...
			return []core.Event{
				core.ToolUseStart{ID: id, Name: done.Name, OutputIndex: done.OutputIndex, ItemID: done.ItemID},
				core.ToolUseDelta{ID: id, OutputIndex: done.OutputIndex, ItemID: done.ItemID, ArgumentsDelta: []byte(done.Arguments)},
				core.ToolUseDone{ID: id, OutputIndex: done.OutputIndex, ItemID: done.ItemID},
			}, nil
		}
		return []core.Event{core.ToolUseDone{ID: s.toolID(done.ItemID), OutputIndex: done.OutputIndex, ItemID: done.ItemID}}, nil
	case "response.completed":
		...
		s.done = true
		return []core.Event{
			core.UsageEvent{Usage: responsesUsageToUsage(completed.Response.Usage, s.model)},
			core.DoneEvent{FinishReason: core.NormalizeFinishReason(completed.Response.Status), FinishReasonRaw: completed.Response.Status, Provider: "openai", Model: s.model},
		}, nil
	case "response.incomplete":
		...
		s.done = true
		finish, rawReason := core.FinishReasonLength, incomplete.Response.Status
		if incomplete.Response.IncompleteDetails != nil && incomplete.Response.IncompleteDetails.Reason != "" {
			...
			finish = core.NormalizeFinishReason(rawReason)
		}
		return []core.Event{
			core.UsageEvent{Usage: responsesUsageToUsage(incomplete.Response.Usage, s.model)},
			core.DoneEvent{FinishReason: finish, FinishReasonRaw: rawReason, Provider: "openai", Model: s.model},
		}, nil
	case "response.failed":
		...
		return []core.Event{core.ErrorEvent{Err: core.NewProviderError("openai", core.ErrorTypeProvider, fmt.Sprintf("openai: response failed: [%s] %s", failed.Response.Error.Code, failed.Response.Error.Message))}}, nil
	case "error":
	...
	default:
		...
		return nil, core.NewProviderError("openai", core.ErrorTypeProvider, "openai: responses stream event missing type")
	}
}
...
func responsesUsageToUsage(u responsesUsage, model string) core.Usage {
	out := core.Usage{
...
func responsesOutputBlocks(items []responsesContentItem) ([]core.Block, error) {
	blocks := make([]core.Block, 0, len(items))
...
func responseAnnotations(item responsesContentItem) ([]core.Annotation, error) {
	if len(item.Annotations) == 0 {
		return nil, nil
	}
	out := make([]core.Annotation, 0, len(item.Annotations))
	for _, raw := range item.Annotations {
		extra, err := marshalRaw(raw)
		if err != nil {
			return nil, fmt.Errorf("openai: marshal response annotation: %w", err)
		}
		ann := core.Annotation{Extra: extra}
